...
export default function ContactsScreen() {
    ...
    try {
      ...
      for (const c of sortedContacts) {
        ...
        if (c.contact_user_id) {
          ...
          const linkedSnap = await getDocs(
            query(
              collection(db, "user_profiles"),
              where("__name__", "==", c.contact_user_id)
            )
          );
          if (!linkedSnap.empty) {
            linked = {
              id: linkedSnap.docs[0].id,
              ...linkedSnap.docs[0].data(),
            };
      ...
      try {
        await AsyncStorage.setItem(
          "cache:contacts",
          JSON.stringify(enrichedContacts)
        );
      } catch {}
    } catch (error: any) {
  ...
  const filterContacts = () => {
    ...
    const contactOptions: ContactOption[] = filteredContactsList.map(
      (contact) => ({
        id: contact.id,
        type: "contact",
        title: `${contact.first_name} ${contact.last_name || ""}`,
        subtitle: contact.contact_user
  ...
  const startChatWithContact = async (contact: Contact) => {
    try {
      ...
      const myChatsSnap = await getDocs(
        query(
          collection(db, "chats"),
          where("participants_auth", "array-contains", user.uid)
        )
      );
      const existing = myChatsSnap.docs.find((d) => {
        ...
        return (
          Array.isArray(parts) &&
          parts.includes(myProfileId) &&
          parts.includes(otherProfileId)
        );
      });
      if (existing) {
        ...
        setTimeout(
          () =>
            (router.push as any)({
              ...
              params: savedName ? { contactName: savedName } : undefined,
            }),
          0
        );
        return;
      ...
      const participantSummaries: Record<string, any> = {
        [myProfileId]: {
          ...
          profile_picture_url:
            (meDoc.data() as any)?.profile_picture_url ||
            (meDoc.data() as any)?.profile_picture_data ||
            null,
        },
      };
      if (otherData) {
        participantSummaries[otherProfileId] = {
          ...
          profile_picture_url:
            (otherData as any)?.profile_picture_url ||
            (otherData as any)?.profile_picture_data ||
            null,
        };
      }
  ...
  const renderContactItem = ({ item }: { item: ContactOption }) => {
    if (item.type === "section") {
      return (
        ...
        >
          <Text
            style={[styles.sectionTitle, { color: theme.colors.mutedText }]}
          >
            {item.title}
          </Text>
        </View>
    ...
    const isQuickTalkUser =
      !!item.contact?.is_quicktalk_user || !!item.contact?.contact_user;
    const profilePicture =
      item.contact?.contact_user?.profile_picture_data ||
      item.contact?.contact_user?.profile_picture_url ||
      item.contact?.contact_user?.profilePicture;
    const quicktalkId = item.contact?.contact_user_id;
    const savedName = `${item.contact?.first_name || ""} ${
    ...
    return (
      ...
      >
        ...
        <View style={styles.contactContent}>
          <Text
            style={[
              ...
              item.type === "contact" &&
                isQuickTalkUser && { color: theme.colors.primary },
            ]}
          >
            {item.title}
          </Text>
          ...
          {!isQuickTalkUser && item.type === "contact" && (
            <Text
              style={[styles.notOnQuicktalk, { color: theme.colors.mutedText }]}
            >
              Not on QuickTalk
            </Text> // 🔹 small note
          )}
        ...
        {isQuickTalkUser && item.type === "contact" && (
          <View
            style={[
              styles.quicktalkBadge,
              { backgroundColor: theme.colors.primary },
            ]}
          >
            <Text
              style={[
                styles.quicktalkBadgeText,
        ...
        {item.type === "contact" && (
          <TouchableOpacity
            ...
            onPress={() => {
              if (item.contact) startChatWithContact(item.contact);
            }}
          >
            <Feather
              name="message-circle"
              size={18}
              color={isQuickTalkUser ? theme.colors.primary : "#999"}
            />
          </TouchableOpacity>
        )}
      </TouchableOpacity>
  ...
  const renderHeader = () => {
    ...
    return (
      ...
      >
        <Text style={[styles.statsText, { color: theme.colors.mutedText }]}>
